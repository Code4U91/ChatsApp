...
import com.example.chatapp.chat_feature.data.local_source.mapper.toEntity
...
class LocalChatRepoImpl(
    ...
    override suspend fun insertChat(chats: List<Chat>) = withContext(dispatcher) {

        chatDao.insertChat(chats.map { it.toEntity() })

    }

    override suspend fun insertMessage(messages: List<Message>) = withContext(dispatcher) {
